...
use App\RaffleDemo\Raffle\Domain\Projection\Raffle\V1\RaffleProjector;
...
final class RaffleProjectorTest extends TestCase
{
    ...
    #[Test]
    public function it_projects_the_ended_event(): void
    {
        // Arrange
        $raffle = RaffleDomainContext::create();
        $raffle = RaffleDomainContext::start($raffle);
        $raffle = RaffleDomainContext::allocateTicketToParticipant($raffle);
        $raffle = RaffleDomainContext::close($raffle);
        $raffle = RaffleDomainContext::end($raffle);
        $events = $raffle->flushEvents();
        ...
        self::assertSame(
            $raffle->ended?->at,
            $this->repository->getById($raffle->getAggregateId()->toString())->endedAt,
        );
    }

    #[Test]
    public function it_does_not_project_ended_event_when_a_raffle_is_not_created(): void
    {
        // Arrange
        $raffle = RaffleDomainContext::create();
        $raffle = RaffleDomainContext::start($raffle);
