...
final readonly class AllocateTicketToParticipantCommandHandler implements CommandHandlerInterface
{
    ...
    public function __invoke(AllocateTicketToParticipantCommand $command): void
    {
        try {
            ...
            $ticketAllocation = $raffle->ticketAllocations->getByHash($command->ticketAllocation->hash);

            $this->transactionBoundary->begin();
            $this->repository->store($raffle);

            $this->domainEventBus->publish(TicketAllocatedToParticipantV1Event::fromNew(
                ...
                allocatedAt: $ticketAllocation->allocatedAt, // @phpstan-ignore-line argument.type
                allocatedQuantity: $ticketAllocation->quantity, // @phpstan-ignore-line argument.type
                allocatedTo: $ticketAllocation->allocatedTo, // @phpstan-ignore-line argument.type
